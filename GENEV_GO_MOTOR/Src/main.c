...
{
	//Monitor whether UART1 is activated if it is not activated, we must not use this function
    //Set Speed Limit and Max Speed
	if(throttle == 1)
	{
        MC_ProgramTorqueRampMotor1(8000,1000);
        motor_move = 1;
	}
	...
	else if(motor_move == 0) //Triggered by UART
	{
		MC_StopMotor1();
		throttle = 0;
	}
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	if(htim == &htim3)
	{
		//Monitor the Speed. You can make use of the monitored speed to set the limitations on the motor
		...
		{
			/*
			 * The motor is not moving, if the motor is spinning, even though the RPM <= 70, user could still set the torque/speed ramp
			 */
			if(motor_move == 0)
			{
				throttle = 0;
			}
			else if (motor_move == 1)
			{
				//DO NOTHING
			}
		}
		...
		if(rpm == 700)
		{
            ...
            {
            	/*
            	 * User no longer set the Torque/Speed Ramp as the motor has reached at the speed limit.
            	 */
            	throttle = 0;
            }
		}
...
uint8_t RX_CMD_BUFFER[4];
//Receive CMD from the user
void HAL_RxCpltCallback(UART_HandleTypeDef *huart)
{
	if(huart==&huart1)
	{
         //Do something
		if(RX_CMD_BUFFER[0] == 0x01)
		{
            //Set the min speed limit for soft start
		}
		else if (RX_CMD_BUFFER[2] == 0x02)
		{
            //User can modify the speed by changing the Iq values
		}
	}
}
